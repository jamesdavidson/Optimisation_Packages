...
for dmsn in range(2,upper_dimension_limit+1): #All sets of n-th dimensional Rosenbrocks
    opt_Ros = Test_Functs.Styb(dmsn) #Creates a Rosenbrock object
    Ros_Comb = function_Block(opt_Ros)
    for x in itt.product(np.arange(univ_low_limit,univ_up_limit,lim_inc),repeat=dmsn):
        Data = open(os.path.join(data_directory,'%d-th dimensional BFGS Styb data.txt' %(dmsn)),'a+')
        #x=[1.1,0.9]
        x0=np.array(x)
        [x_min, iter_no, time] = Optimisers.BFGS(x0,Ros_Comb,iter_limit=300)
